...
class ViewerFragment : Fragment() {
    ...
    private fun showLargeFileWarning(uri: Uri, isStl: Boolean, sizeBytes: Long) {
        ...
        AlertDialog.Builder(requireContext())
            .setTitle(getString(R.string.large_file_warning_title))
            .setMessage(getString(R.string.large_file_warning_message, sizeText))
            .setCancelable(false)
            .setPositiveButton(getString(R.string.large_file_btn_continue)) { _, _ ->
                showLargeFileConfirm(uri, isStl)
            }
            // "رجوع" → إلغاء التحميل تمامًا، رجوع لحالة "لا يوجد ملف محمّل" (ما بيحصلش أي تغيير)
            .setNegativeButton(getString(R.string.large_file_btn_back)) { d, _ -> d.dismiss() }
            .show()
    }
    ...
    private fun showLargeFileConfirm(uri: Uri, isStl: Boolean) {
        ...
        AlertDialog.Builder(requireContext())
            .setTitle(getString(R.string.large_file_confirm_title))
            .setMessage(message)
            .setCancelable(false)
            .setPositiveButton(getString(R.string.large_file_btn_agree)) { _, _ ->
                proceedToLoad(uri, isStl, forceSimplify = isStl)
            }
            // "إلغاء" → إلغاء التحميل، رجوع لحالة "لا يوجد ملف محمّل"
            .setNegativeButton(getString(R.string.large_file_btn_cancel)) { d, _ -> d.dismiss() }
            .show()
    }
